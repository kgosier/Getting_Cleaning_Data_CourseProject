...
#want. Many extra \'s are needed to do this match. Subject and Activity
#columns left out for easier use with aggregate() below

meanstdsub <- grep("\\-mean\\(\\)|\\-std\\(\\)", colname.list)
meanstd.dataset <- all.dataset[,meanstdsub]

#create means by Subject and Activity, and add columns back into
#meanstd.dataset

summary.dataset <- aggregate(meanstd.dataset, 
    by = list(Subject = all.dataset$Subject, Activity = all.dataset$Activity),
    FUN = mean, na.rm=TRUE)
meanstd.dataset <- cbind(all.dataset$Subject, all.dataset$Activity,
    meanstd.dataset)

#clean up a little bit. Let weardata.zip stay in case we need it, but remove
#the unzipped folder
